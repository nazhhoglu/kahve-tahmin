...
gercek_degerler = yeni_veri["Coffee_Consumption_kg"]  # <-- BU SATIRI GEREKİRSE DÜZENLE

# Eksik sütunları ekle, fazlaları çıkar
for col in feature_names:
    if col not in yeni_veri.columns:
        yeni_veri[col] = 0
yeni_veri = yeni_veri[feature_names]
...
sonuc_df.to_csv("rf_tahmin_vs_gercek.csv", index=False)
